...
CREATE TABLE IF NOT EXISTS search_results (
    ...
    price_at_search           REAL,                -- snapshot: listing's price at the moment of THIS search
    currency_at_search        TEXT,                -- snapshot: listing's currency at the moment of THIS search
...
**Why `price_at_search` / `currency_at_search` are denormalized onto `search_results`.** `listings` is intentionally a *current-state* table — UPSERT overwrites `price` (and `raw_blob`) on every harvest so we can answer "what does Marketplace say about this listing now". That is what the user sees when they look at a single listing. But the diff in spec §4.5 needs the *historical* price at search-time T to detect `PRICE_CHANGED` between runs — and once we re-harvest, the prior price is gone from `listings`. So the rule is: snapshots live next to the snapshot. `search_results` is the per-search snapshot, and the price we observed during *that specific* search belongs there. Reading from the gzipped `raw_blob` would have the same staleness problem (UPSERT replaces it too) and would cost a gunzip + JSON parse per row to extract one float — strictly worse. Cost of the columns is a float + a short string per search-result row; trivial. Triggered by bug #6 (AT-5.2 PRICE_CHANGED was permanently empty without this); see §7.9.
...
6. **Within-run duplicate listings (raised by QA test DIFF-7).** Marketplace can return the same `marketplace_id` on two different pages of a single harvest. Decision: **`search_results` has `PRIMARY KEY (search_id, listing_id)`, so within-run dups collapse to one row at the schema level — no application logic required.** The retained row keeps the *first* `position` we encountered (use `INSERT OR IGNORE` on duplicates, not `OR REPLACE`); rationale is that a listing's "rank" is its earliest appearance, and Marketplace re-showing it later is just recency-weighted UI noise. The de-dup is logged once per run as `dedup: dropped N within-run duplicate listings` so it's visible to anyone debugging an unexpected count.
7. **Rate-limit on re-run of the same query (spec §6 Q7, raised by QA test RATE-1).** Decision: **default 5-minute minimum interval between runs of the same `parsed_filters_hash`; configurable via `--min-interval` (seconds), bypass with `--force`.** Implementation: before issuing the search, query `MAX(run_at) WHERE parsed_filters_hash = ?`; if `now - max_run_at < min_interval`, exit with non-zero and a clear "Last run was N seconds ago; minimum interval is M. Use --force to override." This is also restated in §5 Layer 4 (where it lives operationally) — repeating it here so QA has one place to point RATE-1 at.
8. **Currency mismatch (raised by QA test PRICE-10).** Decision: already in §7.3 — listing price in a non-CAD currency fails the price filter with `validation_failures_json` reason `currency_mismatch`. Restating here so QA can map PRICE-10 directly to this ruling.
9. **Price snapshot for diff (raised by bug #6, blocks AT-5.2).** Decision: **add `price_at_search` (REAL) and `currency_at_search` (TEXT) columns to `search_results`; populate at search-record time from the just-harvested `NormalizedListing`, not from a re-read of `listings`.** Diff (`differ.passed_pairs_for_search`) reads price from `search_results`, not from `listings`. Rationale is in §4 (the "Why … denormalized" paragraph) but TL;DR: `listings` is current-state and gets overwritten on UPSERT, so the prior-run price is gone the moment a re-harvest lands; we need a snapshot pinned to the search row. QA's Path B (read from `raw_blob`) hits the same staleness problem and would force a gunzip+parse per row; strictly worse. This is a schema change — bumps `schema_version`, which trips the existing blow-away migration policy. Acceptable: the cache is throwaway by spec design (§6.6) and we're still pre-v1.0. Blow-away message in `init_db()` should mention price-snapshot is the reason, so that any user with an existing cache understands why their DB just got reset.
